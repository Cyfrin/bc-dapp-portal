...
/* 
  This file will look for hyperchain configuration files on the battlechain-era repo
...
const rootPath = process.env.BATTLECHAIN_HOME;
if (!rootPath) {
  console.error("Please set BATTLECHAIN_HOME environment variable to contain path to your battlechain-era repo.");
  process.exit(1);
}
...
const createNetworkFromEnv = (envPath: string): Network => {
  ...
  return {
    id: Number(env.CHAIN_ETH_BATTLECHAIN_NETWORK_ID),
    key: baseName,
    name: env.CHAIN_ETH_BATTLECHAIN_NETWORK,
  ...
  const getEnvsFromDirectory = (directoryPath: string): string[] => {
    if (existsSync(directoryPath)) {
      const envs = readdirSync(directoryPath)
        ...
        .map((file) => file.name);
      if (!envs.length) {
        console.error("No environment files found in your battlechain-era repo. Please set up your hyperchain first.");
        process.exit(1);
      }
      return envs;
    } else {
