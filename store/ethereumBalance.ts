...
export const useEthereumBalanceStore = defineStore("ethereumBalance", () => {
  ...
  const {
    ...
    async () => {
      ...
      if (!networkIdToAnkr.has(bcNetwork.value.l1Network.id)) {
        throw new Error(`Ankr does not support ${bcNetwork.value.l1Network.name}`);
      }
      const balances = await ankrProvider.getAccountBalance({
        blockchain: [networkIdToAnkr.get(bcNetwork.value.l1Network.id)!] as AnkrSupportedChains[],
