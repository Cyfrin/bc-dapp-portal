...
Feature: Artifacts - UI
  ...
  Scenario: Check artifacts for an empty wallet
    ...
    Then Message "Battle Chain Testnet" should be visible
    ...
    Then Element with "href and text" "'/transaction/battlechain/era/receive' and 'Add funds'" should be "visible"
    Then Element with "href and text" "'/transaction/battlechain/era/receive' and 'Add funds'" should be "clickable"
  ...
  Scenario Outline: Check artifacts on tokens dropdown on Deposit/Withdraw/Transfer page for Empty Wallet
    Given I go to page "/transaction/battlechain/era/<transaction>/?network=era-<network>&address=0x47BCD42B8545c23031E9918c3D823Be4100D4e87"
  ...
  Scenario: Check "Insufficient balance" warning message (Zero token balance) (Withdraw)
    Then A wallet should be "empty"
    When I go to page "/transaction/battlechain/era/withdraw"
